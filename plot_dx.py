...
parser.add_argument('--nxs', nargs='+', type=int, default=1.0, help='The number of elements horizontal-wise looping for.')
...
for nx in nxs:
    ...
    for dt in dts:
        deltax = length / nx
        error = _load_or_zeros('error', dt, 'nx', nx)
        residual = _load_or_zeros('residual', dt, 'nx', nx)
        its = len(error)
    ...
    ax.semilogx(dts, it_list, marker='o', label=f'dx={deltax}')
    ax.legend(fontsize=20)
    # ax.set_xlabel('dt')
    ax.set_ylabel('KSP iterations', fontsize=20)
    ax.set_title('KSP iterations vs dt for varying dx')
    ax_scale.semilogx(dts_scaled, it_list, marker='o', label=f'dx={deltax}')
    ax_scale.legend(fontsize=20)
    # ax_scale.set_xlabel('dt / T')
    ax_scale.set_ylabel('KSP iterations', fontsize=20)
    ax_scale.set_title('KSP iterations vs dt/T for varying dx')

    ax_res.semilogx(dts, it_res_list, marker='o', label=f'dx={deltax}')
    ax_res.legend(fontsize=20)
    # ax_res.set_xlabel('dt')
    ax_res.set_ylabel('KSP iterations (residual)', fontsize=20)
    ax_res.set_title('Residual KSP iterations vs dt for varying dx')
    ax_res_scale.semilogx(dts_scaled, it_res_list, marker='o', label=f'dx={deltax}')
    ax_res_scale.legend(fontsize=20)
    # ax_res_scale.set_xlabel('dt / T')
    ax_res_scale.set_ylabel('KSP iterations (residual)', fontsize=20)
    ax_res_scale.set_title('Residual KSP iterations vs dt/T for varying dx')
...
for dt in dts:
    ...
    for nx in nxs:
        ...
        ax_rob.semilogy(x, err_plot, marker='o', label=f'nx={nx}')
        ax_res_rob.semilogy(x_res, res_plot, marker='o', label=f'nx={nx}')
...
# SNES error vs cumulative KSP iteration count, one curve per nx value, per dt.
for dt in dts:
    fig_snes, ax_snes = plt.subplots(figsize=(10, 6))
    has_data = False
    for nx in nxs:
        deltax = length / nx
        snes_err_path = _find_data_path('snes_error', dt, 'nx', nx)
        snes_ksp_path = _find_data_path('snes_ksp_cum', dt, 'nx', nx)
        ...
        ax_snes.semilogy(snes_ksp_cum, snes_err, marker='o', label=f'nx={nx}')
        has_data = True
    if has_data:
        ...
        fig_snes.savefig(f'snes_error_dx_dt{dt}.png')
    plt.close(fig_snes)
